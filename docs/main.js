(function () {
  ...
  const $steepOutput = CodeMirror.fromTextArea(document.getElementById("steep-output"), {
    mode: "text/x-ruby",
    readOnly: true
  });
  ...
  function onAnalyzeClick() {
    ...
    requestAnalyze(rb, rbs, option)
      .then((result) => {
        if (result["status"] == "ok") {
          $steepOutput.getDoc().setValue(result["output"]);
          $minibuffer.value = "";
        }
        else {
  ...
  function onReportBugClick() {
    const rb = $codeRb.getDoc().getValue();
    const rbs = $codeRbs.getDoc().getValue();
    const out = $steepOutput.getDoc().getValue();
    ...
    window.open("https://github.com/mame/steep-playground/issues/new?" + params.toString());
  }
